...
- Click the clone or download button on this [repo](https://github.com/JGordy/react-weather), Or copy the link below.
- https://github.com/JGordy/react-weather.git
- In your terminal go to the directory you want to store this project and type ```git clone https://github.com/JGordy/react-weather.git```
- Once that is done eith type ```npm install``` or ```yarn``` to download the needed dependencies.
- Navigate to the newly created directory and type either ```npm start``` or ```yarn start``` to start the project locally.
- If a browser window doesn't open, type ```localhost:3000``` in your browser bar.
